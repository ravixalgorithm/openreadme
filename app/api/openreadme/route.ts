        ...
        if (existingFilesResponse.ok) {
            ...
            for (const file of userFiles) {
                try {
                    ...
                    console.log(`⚠️ Could not delete old file ${file.path}`);
                }
            }
        }
    } catch (cleanupError) {
...
export async function GET(req: NextRequest) {
    ...
    const g = decodeURIComponent(searchParams.get("g") || "").trim();
    ...
    if (!g || g.length < 2 || g.length > 39) {
        return new NextResponse(
            JSON.stringify({ error: "Valid GitHub username is required (2-39 characters)" }),
    ...
    // Complete HTML template with improved layout and proper height handling
...
<html lang="en">
  ...
  <body class="bg-neutral-950 text-white">
    ...
    try {
        ...
        // Determine environment and launch browser
        ...
        if (isVercel || isProduction) {
            ...
            browser = await puppeteerCore.default.launch({
                ...
                defaultViewport: { width: 1200, height: 1600 }, // Increased height
        ...
        // Set larger viewport to accommodate full content
        await page.setViewport({
            width: 1200,
            height: 1600, // Increased height to fit all content
            deviceScaleFactor: 1.5,
        });

        console.log("📄 Setting page content...");
        await page.setContent(html, { waitUntil: "networkidle0" });
        await new Promise((resolve) => setTimeout(resolve, 2500)); // Wait longer for icons and images to load

        console.log("📸 Taking full page screenshot...");

        // Use fullPage: true to capture everything without clipping
        const screenshot = await page.screenshot({
            type: "png",
            fullPage: true, // This ensures the entire content is captured
        });

        await browser.close();

        console.log("📤 Screenshot completed, uploading to GitHub...");

        // Upload to GitHub with tracking
        try {
            const imageUrl = await uploadToGitHubSafely(
            ...
            console.log(`🎉 Successfully generated and uploaded image for ${g}`);
