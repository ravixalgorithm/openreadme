export const generateContributionGraph = (
    ...
    function getFillColor(count: number): string {
        ...
        if (count <= 5) return "#0e4429"; // Dark green for count <= 5
        if (count <= 10) return "#006d32"; // Medium dark green for count <= 10
        if (count <= 20) return "#26a641"; // Medium green for count <= 20
        return "#39d353"; // Light green for count > 20
    }
